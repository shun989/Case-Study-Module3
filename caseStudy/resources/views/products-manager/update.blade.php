...
<html lang="en">
<head>
    <title>{{__('language.Update Product')}}</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('language.Update Product') }}</div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.update',$oldProduct->id) }}">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="category" value="{{$oldProduct['category']}}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ __('language.Description') }}</label>
                            <textarea name="description" class="form-control" id="exampleFormControlTextarea1" rows="5">{{$oldProduct['description']}}</textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ __('language.Image') }}</label>
                            <input type="text" class="form-control" name="image" value="{{$oldProduct['image']}}">
                        </div>
                        <div class="mb-3 mb-0">
